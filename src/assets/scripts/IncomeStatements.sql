...
SELECT  (CASE WHEN A.MONTH = 1 THEN 'Jan' 
        ...
        ( (NET_MARGINAL_PROFIT + NET_DIVIDEND + NET_INTEREST) / (MARGINAL_PROFIT + DIVIDEND + INTEREST) * 100 ) AS PROFIT_MARGIN_RATE , 
        ( (NET_MARGINAL_PROFIT + NET_DIVIDEND + NET_INTEREST) / (DEPOSIT + RETAINED_EARNINGS) * 100 ) AS ROE , 
        ( (NET_MARGINAL_PROFIT + NET_DIVIDEND + NET_INTEREST) / (LONG_TERM_LIABILITIES) ) AS NET_INCOME_LONG_TERM_LIAB
FROM    my_income_statements A , 
        my_balance_sheets B 
