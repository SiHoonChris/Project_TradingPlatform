    ...
    <div id="income-statement-table">
      <table>        
        ...
        <tbody>
          ...
          <tr><td>Profit Margin Rate</td>            <td v-for="(d, i) in DATA" :key="i">{{DATA[i].PROFIT_MARGIN_RATE.toLocaleString()}}</td></tr>
          <tr><td>ROE</td>                           <td v-for="(d, i) in DATA" :key="i">{{DATA[i].ROE.toLocaleString()}}</td></tr>
          <tr><td>Net Income / Long-term Liab.</td>  <td v-for="(d, i) in DATA" :key="i">{{DATA[i].NET_INCOME_LONG_TERM_LIAB.toLocaleString()}}</td></tr>
        </tbody>
      </table>
    </div>
  </div>
